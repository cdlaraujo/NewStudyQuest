import { QuestResult } from '../quest/QuestResult.js';
...
export class BossContainer {
  ...
  toJSON() {
    return { current: this.#current, completed: this.#completed, quests: this.#quests.map((q) => q.toJSON()) };
  }

  static fromJSON(data, questFactory) {
    const quests = data.quests.map(questFactory);
    const boss = new BossContainer(quests);
    boss.#current = data.current;
    boss.#completed = data.completed;
    return boss;
  }
}
